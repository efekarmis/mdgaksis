<%@ Page Title="Ders Mesajlarım" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="OgrenciMesajlar.aspx.cs" Inherits="YazOkulu.OgrenciMesajlar" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder2" runat="server">
    ...
    <div class="mesajlasma-container shadow-sm border rounded">
        ...
        <div class="ders-listesi-panel">
            <div class="ders-liste-baslik p-3 border-bottom">
                <h5 class="mb-0 fw-semibold"><span class="glyphicon glyphicon-list-alt me-2"></span>Kayıtlı Derslerim</h5>
            </div>
            <div class="ders-liste-icerik" style="overflow-y: auto;">
                <asp:Repeater ID="rptDersListesiSolMenuOgr" runat="server" OnItemCommand="rptDersListesiSolMenuOgr_ItemCommand">
                    <ItemTemplate>
                        <asp:LinkButton ID="lnkDersSecOgr" runat="server"
                            CssClass='<%# "ders-list-item p-3 border-bottom " + (Eval("DersId").ToString() == hfSeciliDersId.Value ? "active" : "") %>'
                            CommandName="DersSec" CommandArgument='<%# Eval("DersId") %>'
                    ...
                    <FooterTemplate>
                        <% if (rptDersListesiSolMenuOgr.Items.Count == 0)
                            { %>
                        <div class="p-3 text-center text-muted">Kayıtlı olduğunuz ders bulunmamaktadır.</div>
                        <% } %>
                    </FooterTemplate>
                </asp:Repeater>
            </div>
            ...
            <asp:Panel ID="pnlMesajlasmaAlaniSag" runat="server" CssClass="mesaj-alani-dolu d-flex flex-column h-100" Visible="false">
                ...
                <div id="mesajAkisiContainer" clientidmode="Static" class="mesaj-akisi-container flex-grow-1 p-3" style="overflow-y: scroll;">
                    <asp:Repeater ID="rptMesajAkisi" runat="server">
                        <ItemTemplate>
                            <%-- Öğrenci kendi gönderdiyse (tip 0 ve ID'si Session ile eşleşiyorsa) 'message-sent' --%>
                            <div class='message-item mb-3 <%# Convert.ToInt32(Eval("GonderenKullaniciTipi")) == 0 && Convert.ToInt32(Eval("GonderenKullaniciID")) == (int)(Session["KullaniciID"] ?? 0) ? "message-sent" : "message-received" %>'>
    ...
    <%-- Özel Mesajlaşma Stilleri (styles.css'e taşınmalı - Öğretmenle aynı, sadece gönderilen renk farklı) --%>
    ...
    <script type="text/javascript">
        function scrollToBottom(containerId) {
            ...
            if (div) { setTimeout(function () { div.scrollTop = div.scrollHeight; }, 50); }
        }
        ...
        if (typeof (Sys) !== 'undefined' && Sys.WebForms && Sys.WebForms.PageRequestManager) {
            ...
            prm.add_endRequest(function (s, e) { setTimeout(function () { scrollToBottom('mesajAkisiContainer'); }, 150); });
        }
    </script>
</asp:Content>
